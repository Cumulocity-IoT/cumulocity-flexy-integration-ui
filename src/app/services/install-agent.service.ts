...
export class InstallAgentService {
  ...
  private async loadFilesOntoDevice(
    ...
    try {
      // connector
      const connector = await this.loadFile(
        device.name,
        index,
        '4.1',
        device.encodedName,
      ...
      // JVMrun
      const jvmrun = await this.loadFile(
        device.name,
        index,
        '4.2',
        device.encodedName,
      ...
      if (config.url.hasOwnProperty('cumulocity') && !!config.url.cumulocity) {
        const c8yconfig = await this.loadFile(
          device.name,
          index,
          '4.3',
          device.encodedName,
  ...
  private async pollForDeviceRegistration(
    ...
    for (let i = 1; i <= attempts; i++) {
      ...
      if (
        openRegistration &&
        openRegistration.status === DeviceRegistrationStatus.PENDING_ACCEPTANCE
      ) {
